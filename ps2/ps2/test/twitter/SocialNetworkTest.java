...
public class SocialNetworkTest {
    ...
    @Test
    public void testGuessFollowsGraphMultipleTweetsMultipleMentionsMultipleUsersMultipleUsers() {
        List<Tweet> tweets = Arrays.asList(tweet3, tweet4, tweet5, tweet6, tweet7, tweet8, tweet9, tweet11);
        Map<String, Set<String>> followsGraph = SocialNetwork.guessFollowsGraph(tweets);
        
        boolean falseKey = false;
        for (Map.Entry<String, Set<String>> entry : followsGraph.entrySet()) {
            if (!entry.getKey().equalsIgnoreCase("alyssa") &&
                    !entry.getKey().equalsIgnoreCase("bbit-diddle") &&
                    !entry.getKey().equalsIgnoreCase("mvg180") &&
                    !entry.getKey().equalsIgnoreCase("guess_not")) {
                falseKey = true;
            }
            for (String user : entry.getValue()) {
                if (!user.equalsIgnoreCase("alyssa") &&
                        !user.equalsIgnoreCase("bbit-diddle") &&
                        !user.equalsIgnoreCase("mvg180") &&
                        !user.equalsIgnoreCase("guess_not")) {
                    falseKey = true;
                }
            }           
        }
        assertFalse("did not expect this value", falseKey);
        ...
        assertTrue("expected map to contain user", followsGraph.keySet().stream().anyMatch("mvg180"::equalsIgnoreCase));
        assertEquals("expected size 2", 2, followsGraph.get(followsGraph.keySet().stream().filter("alyssa"::equalsIgnoreCase).findFirst().get()).size());
        assertEquals("expected size 1", 1, followsGraph.get(followsGraph.keySet().stream().filter("mvg180"::equalsIgnoreCase).findFirst().get()).size());
    ...
    // Covers adds non existent users
    @Test
    public void testGuessFollowsGraphAddsNonExistentUsersBug() {
        Map<String, Set<String>> followsGraph = SocialNetwork.guessFollowsGraph(Arrays.asList(tweet2, tweet12));
    ...
    // Covers influencers implementation must be case insensitive
    @Test
